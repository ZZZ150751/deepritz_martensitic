...
def train(model, device, params):
    ...
    for epoch in range(1, params["num_epochs"] + 1):
        ...
        xb = xb.detach()        #切断梯度反向传播

        output_r = model(xr)
        output_b = model(xb) #(4 * b_batch_size, 1)维张量

        u_b = params["gamma"] * xb[:, 0:1]
        ...
        grad_xu = torch.autograd.grad(outputs=grad_x, inputs=xr,
        ...
        # 能量损失
        loss_e = torch.mean(W(grad_x, grad_y) + ((epsilon(params) ** 2) * 0.5 ) * (grad_xx ** 2))
...
def main():
    ...
    for name, t in epsilon_dict.items():
        print(f"\nTraining with {name}.")
        params["t"] = t
        params["model_name"] = f"epsilon{t}"
        ...
        # 取 u_x
        ux_pred = grad_u[:, 0:1].detach().cpu().numpy().reshape(len(xr_t), len(yr_t))

        grad_x = grad_u[:, 0:1]
        grad_y = grad_u[:, 1:2]

        grad_xu = torch.autograd.grad(outputs=grad_x, inputs=points,
                                      grad_outputs=torch.ones_like(grad_x),
                                      create_graph=True)[0]
        grad_xx = grad_xu[:, 0:1]

        integrand = W(grad_x, grad_y) + 0.5 * (epsilon(params) ** 2) * (grad_xx ** 2)
        ...
        h = math.log(0.1 / epsilon(params), 2)
        layers.append(h)
        ...
        plt.savefig(f'10_epsilon{t}.png', dpi=300, bbox_inches='tight')
        plt.close()

    plt.figure(figsize=(8, 6))
    plt.plot(layers, energy_list, marker='o',linewidth=2,
                    color="green")
    plt.xlabel("log_2(0.1/epsilon)")
    plt.ylabel("E")
    plt.title("γ=0.25")
    plt.grid(True)
    plt.savefig("Fig10_a.png", dpi=300, bbox_inches='tight')
    plt.show()
