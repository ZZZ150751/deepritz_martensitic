...
class DNN(nn.Module):
    ...
    # 权重和偏置初始化的函数（截断正态分布，方差为：2/(dim_in + dim_out)）
    @staticmethod  # 在类中
...
def train(model, device, params):
    ...
    for epoch in range(1, params["num_epochs"] + 1):
        ...
        if epoch > int(0.75 * params["num_epochs"]):
            if loss.item() < best_loss:
                best_loss = loss.item()
                best_epoch = epoch
                torch.save(model.state_dict(), f'model_{params["activation_name"]}.mdl')

        if epoch % 1000 == 0:
            print(f'Epoch {epoch}/{params["num_epochs"]}, Loss: {loss.item():.6f}, '
                  f'Energy Loss: {loss_e.item():.6f}, Boundary Loss: {loss_b.item():.6f}')

    print(f'Activation={params["activation_name"]}, Best epoch={best_epoch}, Best loss={best_loss:.6f}')
...
def main():
    ...
    for name, activation in activations.items():
        ...
        params["activation_name"] = name

        model = DNN(params["d"], params["m"], params["n"], activation).to(device)
        model.apply(DNN.init_trunc_xavier)
        trained_model = train(model, device, params)

        trained_model.load_state_dict(torch.load(f'model_{params["activation_name"]}.mdl'))
        trained_model.eval()
