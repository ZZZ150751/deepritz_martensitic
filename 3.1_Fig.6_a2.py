...
def pre_train(model, device, params):
    optimizer = Adam(model.parameters(), lr=params["lr"])
    model.train()

    x = torch.linspace(0, 1, 200, device=device).reshape(-1, 1)
    y = params["gamma"] * x + 0.1 * torch.sin(4 * x)

    for step in range(1, params["preStep"] + 1):
        optimizer.zero_grad()
        ...
        if step % 1000 == 0:
            print(f"step: {step}/{params['preStep']}, pre_loss: {loss.item():.6f}")
...
def train(model, device, params):
    ...
    for epoch in range(1, params["num_epochs"] + 1):
        ...
        #能量损失
        loss_e = torch.mean(W(grad_u) + ((params["epsilon"] ** 2) / 2) * (grad_u2 ** 2))
...
def main():
    ...
    for name, epsilon in epsilon_dict.items():
