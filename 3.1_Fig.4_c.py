...
def set_seed(seed: int):
    random.seed(seed)           # 1. Python 内置随机模块
    np.random.seed(seed)        # 2. NumPy 随机数生成器
    torch.manual_seed(seed)     # 3. PyTorch CPU 随机数生成器
    torch.cuda.manual_seed_all(seed)  # 4. PyTorch 所有GPU的随机数生成器
    ...
    for epoch in range(1, params["num_epochs"] + 1):
        ...
        if epoch > int(0.75 * params["num_epochs"]):
            if loss.item() < best_loss:
                best_loss = loss.item()
                best_epoch = epoch
                torch.save(model.state_dict(), f'model_{params["lr"]}.mdl')
...
def main():
    ...
    for name, lr in lr_dict.items():
        ...
        trained_model.load_state_dict(torch.load(f'model_{params["lr"]}.mdl'))
        trained_model.eval()
