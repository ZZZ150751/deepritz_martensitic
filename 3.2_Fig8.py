...
#计算E的取点方法：网格取点，得到 （batch ** 2）x 2维张量
...
def main():
    ...
    for name, n in DNN_dict.items():
        print(f"\nTraining with {name} layers.")
        params["n"] = n
        params["model_name"] = f"heading_layer{n}"
        ...
        plt.savefig(f'ux_depth{n}.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    plt.title("γ=0.5")
    plt.grid(True)
    plt.savefig("Fig8_a.png", dpi=300, bbox_inches='tight')
    plt.show()
